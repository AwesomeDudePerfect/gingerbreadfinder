...
function jumpToServer()
    ...
    if config.servers.pageDeep > 1 then
        for i = 1, config.servers.pageDeep, 1 do
            ...
            body = HttpService:JSONDecode(req.Body) task.wait(0.1)
        end
    end
...
for eventId, event in activeEvents do
    if event.name == config.eventName then
        isGingerbreadExist = true -- tp to event owner tpToPos(event.origin + Vector3.new(0, 18, 0)) 
    end
end
...
if isGingerbreadExist then
    Library.Alert.Message("Gingerbread exist!") -- todo 
    task.wait(config.delays.beforeBreak) -- finded gingerbread breakable 
    ...
    CollectAllLootbags()
    task.wait(config.delays.afterBreak) CollectAllLootbags()
